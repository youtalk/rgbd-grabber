...
void onNewColorSample(ColorNode node, ColorNode::NewSampleReceivedData data) {
    ...
    cv::Mat maxDist = cv::Mat::ones(g_color.rows, g_color.cols,
                                    CV_16UC1) * MAX_DEPTH;
    cv::Mat minDist = cv::Mat::ones(g_color.rows, g_color.cols,
                                    CV_16UC1) * MIN_DEPTH;
    ...
    scaledDepth -= minDist;
    scaledDepth.convertTo(scaledDepth, CV_8UC1,
    ...
    CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_NORMALIZE_IMAGE)
        ...
        if (key == 't') {
            ...
            ss_c << FLAGS_folder << "/" << FLAGS_color << imageNum
                 << FLAGS_type;
            ss_d << FLAGS_folder << "/" << FLAGS_depth << imageNum
                 << FLAGS_type;
        ...
        cv::rectangle(g_color, cv::Point(5, 5), cv::Point(635, 475),
                      cv::Scalar(255, 0, 0), 3);
    } else
        cv::rectangle(g_color, cv::Point(5, 5), cv::Point(635, 475),
                      cv::Scalar(0, 0, 255), 3);
