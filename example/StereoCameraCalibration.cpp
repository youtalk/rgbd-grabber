...
int findChessboard(
    ...
    for (int i = 0; i < lefts.size(); ++i) {
        ...
        if (cv::findChessboardCorners(
            ...
            cv::cornerSubPix(
                    lefts[i], imagePoints[0][i],
                    cv::Size(11, 11), cv::Size(-1, -1),
                    cv::TermCriteria(CV_TERMCRIT_ITER + CV_TERMCRIT_EPS, 30, 0.01));
            cv::cornerSubPix(
                    rights[i], imagePoints[1][i],
                    cv::Size(11, 11), cv::Size(-1, -1),
                    cv::TermCriteria(CV_TERMCRIT_ITER + CV_TERMCRIT_EPS, 30, 0.01));

            cv::drawChessboardCorners(lefts[i], patternSize,
                                      (cv::Mat)(imagePoints[0][i]), true);
            cv::drawChessboardCorners(rights[i], patternSize,
                                      (cv::Mat)(imagePoints[1][i]), true);
            cv::Mat lc, rc;
            cv::resize(lefts[i], lc, cv::Size(lefts[i].cols / 2, lefts[i].rows / 2));
            cv::resize(rights[i], rc, cv::Size(rights[i].cols / 2, rights[i].rows / 2));
            cv::imshow("Left", lc);
            cv::imshow("Right", rc);
        } else {
