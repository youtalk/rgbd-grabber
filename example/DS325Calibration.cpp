...
void loadImages(cv::vector<cv::Mat> &rgb, cv::vector<cv::Mat> &depth,
                const int &fileNum);
int findChessboard(
        cv::vector<cv::Mat> &rgb, cv::vector<cv::Mat> &depth,
...
void loadImages(cv::vector<cv::Mat> &rgb, cv::vector<cv::Mat> &depth,
    ...
    for (int i = 0; i < fileNum; ++i) {
        ...
        cout << "loading : " << rgbfilename.str() << " and "
             << depthfilename.str() << endl;
        ...
        // load depth image
        cv::Mat tempDepth = cv::imread(depthfilename.str(),
        ...
        cv::resize(tempDepth(cv::Rect(40, 43, 498, 498 / 4 * 3)), roiTempDepth,
                   cv::Size(640, 480));
...
int findChessboard(
    ...
    for (int i = 0; i < rgb.size(); ++i) {
        ...
        if (cv::findChessboardCorners(rgb[i], patternSize, imagePoints[0][i],
        CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_NORMALIZE_IMAGE)
            ...
            cv::cornerSubPix(
                    ...
                    cv::Size(11, 11),
                    cv::Size(-1, -1),
                    cv::TermCriteria(CV_TERMCRIT_ITER + CV_TERMCRIT_EPS, 30,
                                     0.01));
            ...
            cv::cornerSubPix(
                    ...
                    cv::TermCriteria(CV_TERMCRIT_ITER + CV_TERMCRIT_EPS, 30,
                                     0.01));
            // 検出点を描画する
            cv::drawChessboardCorners(rgb[i], patternSize,
                                      (cv::Mat) (imagePoints[0][i]), true);
            cv::drawChessboardCorners(depth[i], patternSize,
...
int main(int argc, char *argv[]) {
    ...
    double rms1 = calibrateCamera(worldPoints, imagePoints[0], rgb[0].size(),
                                  cameraMatrix[0], distCoeffs[0], rvecs, tvecs,
                                  CV_CALIB_FIX_K4 | CV_CALIB_FIX_K5);

    double rms2 = calibrateCamera(worldPoints, imagePoints[1], rgb[0].size(),
    ...
    double rms = stereoCalibrate(
            ...
            distCoeffs[0], cameraMatrix[1], distCoeffs[1], rgb[0].size(), R, T,
            E, F,
    ...
    for (int i = 0; i < fileNum; i++) {
        ...
        for (int j = 0; j < npt; j++) {
            double errij =
                    std::fabs(
                            imagePoints[0][i][j].x * lines[1][j][0] + imagePoints[0][i][j].y
                                    * lines[1][j][1]
                            + lines[1][j][2])
                    + fabs(imagePoints[1][i][j].x * lines[0][j][0] + imagePoints[1][i][j].y
                            * lines[0][j][1]
                           + lines[0][j][2]);
            err += errij;
        }
    ...
    stereoRectify(cameraMatrix[0], distCoeffs[0], cameraMatrix[1],
                  distCoeffs[1], rgb[0].size(), R, T, R1, R2, P1, P2, Q,
                  cv::CALIB_ZERO_DISPARITY, 1, rgb[0].size(), &validRoi[0],
                  &validRoi[1]);
    ...
    cv::calibrationMatrixValues(cameraMatrix[0], rgb[0].size(), apertureWidth,
